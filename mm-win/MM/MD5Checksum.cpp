...
tstring CMD5Checksum::GetMD5(const tstring& strFilePath)  
{    
	FILE *fp;
	char ch;
	if((fp=fopen(strFilePath.c_str(),"rb"))==NULL)
	{
    ...
    char Buffer[nBufferSize];   //buffer for data read from the file  
  
    ////checksum the file in blocks of 1024 bytes  
   // while ((nLength = file.Read( Buffer, nBufferSize )) > 0 )
	while(nLength = fread(Buffer, nBufferSize, 1, fp)>0)
    {   
    ...
    tstring strTmp = MD5Checksum.Final();  

	char md5_32[33] = {0};
	int passlen = strTmp.length();
	md5_string((const unsigned char*)strTmp.c_str(), md5_32, passlen);

	tstring strResult = md5_32;
	return strResult;
}  
...
{   
    //check that unsigned int is 4 bytes long - true in Visual C++ 6 and 32 bit Windows  
    //ASSERT( sizeof(x) == 4 );  
...
RETURNS:  void 
ARGUMENTS:  unsigned int* Output : the 32 bit (unsigned long) destination array  
...
of the output unsigned int, the second char to bits 8-15 etc.  
...
*****************************************************************************************/  
void CMD5Checksum::ByteToDWord(unsigned int* Output, char* Input, unsigned int nLength)  
{   
    //entry invariants  
    //ASSERT( nLength % 4 == 0 );  
...
*****************************************************************************************/  
void CMD5Checksum::Transform(char Block[64])  
{   
...
*****************************************************************************************/  
void CMD5Checksum::DWordToByte(char* Output, unsigned int* Input, unsigned int nLength )  
{   
    //entry invariants  
    //ASSERT( nLength % 4 == 0 );  
...
RETURNS:  tstring : the final hexadecimal MD5 checksum result  
ARGUMENTS:  None 
...
{   
    ...
    //Convert the hexadecimal checksum to a tstring  
    tstring strMD5;  
...
*****************************************************************************************/  
void CMD5Checksum::Update( char* Input, unsigned long nInputLen )  
{   
...
tstring CMD5Checksum::GetMD5OfString(tstring strString)  
{    
    CMD5Checksum MD5Checksum;   //checksum object   
    int nLength = strString.length();       //number of bytes read from the file  
    //const int nBufferSize = 1024; //checksum the file in blocks of 1024 bytes  
    char *Buffer;   //buffer for data read from the file  
    Buffer=(char*)(strString.data());  
    //checksum the file in blocks of 1024 bytes  
    //while ((nLength = File.Read( Buffer, nBufferSize )) > 0 )  
    //{   
		MD5Checksum.Update( Buffer, nLength );  
