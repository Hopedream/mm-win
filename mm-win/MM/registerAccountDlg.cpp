...
//			tstring::npos != root["code"].asString().find("1001")
	...
	{
		CMsgBox* pMsgBox = new CMsgBox(_T("错误"),_T("发送网络注册请求失败，请稍候重试！"), MSGBOX_ICON_ERROR);
		pMsgBox->ShowModal();
	}
...
{	
	...
	{
		CMsgBox::MsgBox(this->GetHWND(), _T("注册"), _T("注册之前请认真阅读MM使用协议"), MSGBOX_ICON_WARNING);
		return false;
	}
...
void CRegisterAccountDlg::GetVerifyCode()
{
	...
	if (CWebClient::IsValidUser(strPhone))
	{
		...
		m_pLabelPhoneTip->SetText(_T("该手机号码已经被注册，请直接登录"));
		m_pLabelPhoneTip->SetTextColor(WARNING_TEXT_COLOR);
		return;
	...
	{
		m_pLabelPhoneTip->SetText(_T("网络超时，获取验证码失败，请重试！"));
		m_pLabelPhoneTip->SetTextColor(WARNING_TEXT_COLOR);
		return;
	}
...
//			if (nResult == 1)
		...
		if (bSucc)
		{
			...
			CDuiString strMsg;
			strMsg.Format(_T("剩余%d秒"), m_nPinLifecycle);
	...
	{
		m_pLabelPhoneTip->SetText(_T("发送网络请求失败，请重试！"));
		m_pLabelPhoneTip->SetTextColor(WARNING_TEXT_COLOR);
		return;
	}
...
void CRegisterAccountDlg::GetVerifyCodeFromCss()
{
		...
		&& strHttpResponse.find(_T("vcode")) != -1)
	{
		...
		{
			tstring str;
			str = _T("从CSS获取验证码失败,Http应答:  ") + strHttpResponse;
...
//			pMsgBox->ShowModal();
		...
		{
			tstring str;
			str = _T("查询重置验证码错误，错误代码：") + ret_code;
			CMsgBox* pMsgBox = new CMsgBox(_T("错误"), str, MSGBOX_ICON_ERROR);
			pMsgBox->ShowModal();
			return;
		}
	}
	...
	{
		tstring str;
		str = _T("发送网络请求失败，") + strHttpResponse;
		CMsgBox* pMsgBox = new CMsgBox(_T("错误"), str, MSGBOX_ICON_ERROR);
		pMsgBox->ShowModal();
//		return;
	}
...
//	strUrl += _T("sign-up");
	...
	if (strHttpResponse.find(_T("retcode")) != -1 && strHttpResponse.find(_T("retmsg")) != -1)
	{
		...
		{
			tstring str;
			str = _T("注册失败,Http应答:  ") + strHttpResponse;
			CLogger::GetInstance()->PrintErrLog(str.c_str());		
		}

		if (_T("0") == ret_code)
		{
			CMsgBox* pMsgBox = new CMsgBox(_T("重置支付密码"),_T("重置支付密码成功！"), MSGBOX_ICON_ERROR);
			pMsgBox->ShowModal();
		}
		else
		{
