						 ...
						 int nFaceIndex, COLORREF clrBg, BOOL bAutoScale, int nReservedWidth)
{
	...
	int n = _tcslen(lpszFileName);
	szFileName = new WCHAR[n+1];
	mbstowcs(szFileName,lpszFileName,n);
			...
			if (CLSID_ImageOle == reobject.clsid)
			{
				...
				if (SUCCEEDED(hr))
				{
					...
					else
					{
						...
						TCHAR* strFileName = NULL;
						int n = wcslen(bstrFileName);
						strFileName = new TCHAR[n+1];
						wcstombs(strFileName,bstrFileName,n);
						strText += strFileName;
						::SysFreeString(bstrFileName);
						strText += _T("\"]");
					}
