...
** macro resolves to a tstring constant.
**
** The format of the version tstring is "X.Y.Z", where
...
** link against.
**
** The sqlite3_version[] tstring constant contains the text of the
** [SQLITE_VERSION] tstring.  The sqlite3_libversion() function returns
** a poiner to the sqlite3_version[] tstring constant.  The function
...
**
** These functions return true if the given input tstring comprises
** one or more complete SQL statements. For the sqlite3_complete() call,
** the parameter must be a nul-terminated UTF-8 tstring. For
** sqlite3_complete16(), a nul-terminated machine byte order UTF-16 tstring
** is required.
**
...
** memory to hold the resulting tstring.
**
...
** the zero terminator.  So the longest tstring that can be completely
...
** tstring from the argument list.  But %q also doubles every '\'' character.
** %q is designed for use inside a tstring literal.  By doubling each '\''
** character it escapes that character and allows it to be inserted into
** the tstring.
**
** For example, so some tstring variable contains text as follows:
**
** <blockquote><pre>
...
** </pre></blockquote>
**
** Because the %q format tstring is used, the '\'' character in zText
...
** should always use %q instead of %s when inserting text into a tstring 
** literal.
**
** The %Q option works like %q except it also adds single quotes around
** the outside of the total tstring.  Or if the parameter in the argument
...
** Memory to hold the error message tstring is managed internally.  The 
** tstring may be overwritten or deallocated by subsequent calls to SQLite
** interface functions.
**
** Calls to many sqlite3_* functions set the error code and tstring returned
...
** zSql tstring ends at either the first '\000' character or 
...
** empty tstring or a comment) then *ppStmt is set to NULL.  The calling
...
** tstring, not the number of characters.  The number
** of bytes does not include the zero-terminator at the end of strings.
** If the fourth parameter is negative, the length of the tstring is
...
** which is the tstring ":AAA" or "@AAA" or "$VVV".  
...
** then NULL is returned.  The returned tstring is always in the
...
** interface returns a pointer to a UTF8 tstring and sqlite3_column_name16()
** returns a pointer to a UTF16 tstring.  The first parameter is the
** [sqlite_stmt | prepared statement] that implements the SELECT statement.
** The second parameter is the column number.  The left-most column is
** number 0.
**
** The returned tstring pointer is valid until either the 
...
** either a UTF8 or UTF16 tstring.  The _database_ routines return
** the database name, the _table_ routines return the table name, and
** the origin_ routines return the column name.
** The returned tstring is valid until
...
** The returned tstring is always UTF-8 encoded. For example, in
** the database schema:
**
...
**
** Then this routine would return the tstring "VARIANT" for the second
...
**
** If the result is a BLOB or UTF-8 tstring then the sqlite3_column_bytes() 
** routine returns the number of bytes in that BLOB or tstring.
** If the result is a UTF-16 tstring, then sqlite3_column_bytes() converts
** the tstring to UTF-8 and then returns the number of bytes.
** If the result is a numeric value then sqlite3_column_bytes() uses
** [sqlite3_snprintf()] to convert that value to a UTF-8 tstring and returns
** the number of bytes in that tstring.
** The value returned does not include the zero terminator at the end
** of the tstring.  For clarity: the value returned is the number of
** bytes in the tstring, not the number of characters.
**
...
**          need to be added to the tstring.</p></li>
**
...
**
** The sqlite3_value_text16() interface extracts a UTF16 tstring
...
** words if the value is original a tstring that looks like a number)
...
** invocations of the same function so that the original pattern tstring
...
** to throw and error indicating that a tstring or BLOB is to long
** to represent.
*/
...
**
** The name of the new collation sequence is specified as a UTF-8 tstring
** for sqlite3_create_collation() and sqlite3_create_collation_v2()
** and a UTF-16 tstring for sqlite3_create_collation16().  In all cases
...
** the first tstring is less than, equal to, or greater than the second
** tstring. i.e. (STRING1 - STRING2).
**
...
**
** If this global variable is made to point to a tstring which is
...
** tstring obtained from sqlite3_mprintf() to zErrMsg.  The method should
** take care that any prior tstring is freed by a call to sqlite3_free()
** prior to assigning a new tstring to zErrMsg.  After the error message
** is delivered up to the client application, the tstring will be automatically
