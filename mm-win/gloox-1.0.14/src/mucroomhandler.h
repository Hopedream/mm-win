...
namespace gloox
{
  ...
  class GLOOX_API MUCRoomHandler
  {
    public:
	  ...
	  /**
	  * Add by lee
	  * When app start, we need to get room list of mine.
	  **/
	  virtual void HandleRoomList(MUCRoom* room, const Disco::ItemList& items) = 0;

	  /**
	  * Add by lee
	  * get room member list.
	  */
	  virtual void HandleRoomMemberList(MUCRoom* room, const Disco::ItemList& items) = 0;

	  /**
	  * Add by lee
	  * callback of getting room name by room id
	  */
	  virtual void HandleRoomNameReturn( MUCRoom* room, int features, const std::string& name,
										 const DataForm* infoForm) = 0;
