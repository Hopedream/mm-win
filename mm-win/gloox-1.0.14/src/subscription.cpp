...
namespace gloox
{
    ...
    : Stanza( tag ), m_subtype( Invalid ), m_stati( 0 )
  {
	...
	//lee add
	Tag *t  = tag->findChild("nick");
	if (t)
		m_strSenderNick = t->cdata();
	//m_strSenderNick = (tag->findAttribute("nick"));
  ...
  void Subscription::SetSenderNick(const std::string& strNick)
  {
	  m_strSenderNick = strNick;
  }
  ...
  Tag* Subscription::tag() const
  {
	...
	//lee add: set nick nick.
	//t->addAttribute( "nick", m_strSenderNick);

	new Tag( t, "nick", m_strSenderNick );
