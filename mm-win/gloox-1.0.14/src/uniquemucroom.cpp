...
//       m_parent->removeStanzaExtension( ExtMUCUnique ); // don't remove, other rooms might need it
  ...
  void UniqueMUCRoom::RequestUniqueRoomName()
  {
	  if( !m_parent || m_joined )
		  return;
    ...
    switch( iq.subtype() )
    {
      case IQ::Result:
        if( context == RequestUniqueName )
        {
		  ...
		  if (NULL != m_roomConfigHandler)
		  {
			  m_roomConfigHandler->OnRequestRoomResult(this, true, u->name());
		  }
        }
		else if (SendRoomConfig == context)
		{
			if (NULL != m_roomConfigHandler)
			{
				m_roomConfigHandler->OnCreateRoomResult(this, true);
			}
			
			//MUCRoom::AddGroupMember();
		}
      ...
      case IQ::Error:
        if( context == RequestUniqueName )
        {
		  ...
		  if (NULL != m_roomConfigHandler)
		  {
			  m_roomConfigHandler->OnRequestRoomResult(this, false, s.hex());
		  }
		  
        }
		else if (context == SendRoomConfig)
		{
			if (NULL != m_roomConfigHandler)
			{
				m_roomConfigHandler->OnCreateRoomResult(this, false);
			}
