...
namespace gloox
{
  ...
  const std::string GLOOX_VERSION           = "1.0.14";
...
const char* gloox_get_addr(const char * strKey)
{
	char strTemp[MAX_PATH] = {0};
	GetModuleFileName(NULL,strTemp,MAX_PATH);
	(strrchr(strTemp, '\\'))[1] = 0;
	strcat(strTemp,"config\\host.sys");
	char strValue[MAX_PATH] = {0};
	GetPrivateProfileString("HOST_URL",strKey,"opration error",strValue,MAX_PATH,strTemp);
	return strValue;
}