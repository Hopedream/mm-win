...
//       m_parent->removeStanzaExtension( ExtMUCOwner );
  ...
  void MUCRoom::GetRoomList()
  {
	  if( m_parent )
		  m_parent->disco()->getDiscoItems( JID("muc.test-d"), EmptyString, this, RequestRoomList );
  }
  void MUCRoom::GetRoomMembers(std::string& strRoomID)
  {
	  std::string strNick = strRoomID+"@muc.test-d";

	  if( m_parent )
		  m_parent->disco()->getDiscoItems( JID(strNick), EmptyString, this, GetMembersOfRoom );
  }
  void MUCRoom::GetRoomInfoEx(std::string& strRoomId)
  {
	  std::string strRoomNick = strRoomId+"@muc.test-d";
	  if (m_parent)
	  {
		  m_parent->disco()->getDiscoInfo( JID(strRoomNick), EmptyString, this, GetSpecifyRoomInfo );
	  }
  }
  ...
  void MUCRoom::AddGroupMember(const std::vector<std::string>& vecMembers)
  {
	  std::string strRoomName = name();
	  std::string strServer = m_nick.server();
	  std::string strRoomAddr = strRoomName + "@";
	  strRoomAddr += strServer;

	  IQ iq( IQ::Set, strRoomAddr);
	  MUCListItemList list;
	  if (vecMembers.size()>0)
	  {
		  for (int i=0; i<vecMembers.size(); i++)
		  {
			  const std::string& strAccount = vecMembers[i];
			  int nIndx = strAccount.find("@");
			  std::string strMemNick = strAccount.substr(0, nIndx);
			  list.push_back(MUCListItem(JID(strAccount.c_str()), RoleInvalid, AffiliationAdmin, strMemNick.c_str()));
		  }
		  
	  }
	 
	  iq.addExtension( new MUCAdmin(InitGroupMembers,list));

	  m_parent->send( iq, this, InitGroupMembers );
  }

  void MUCRoom::ExitFromGroup( const std::vector<std::string>& vecMembers )
  {
	  std::string strRoomName = name();
	  std::string strServer = m_nick.server();
	  std::string strRoomAddr = strRoomName + "@";
	  ...
	  if (vecMembers.size()>0)
	  {
		  for (int i=0; i<vecMembers.size(); i++)
		  {
			  ...
			  int nIndx = strAccount.find("@");
			  std::string strMemNick = strAccount.substr(0, nIndx);
			  list.push_back(MUCListItem(JID(strAccount.c_str()), RoleInvalid, AffiliationNone, ""));
		  }

	  }

	  iq.addExtension( new MUCAdmin(ExitRoom,list));

	  m_parent->send( iq, this, ExitRoom );
// 	  std::string strRoomName = name();
// 	  std::string strServer = m_nick.server();
// 	  std::string strRoomAddr = strRoomName + "@";
// 	  strRoomAddr += strServer;
...
// 			  int nIndx = strAccount.find("@");
// 			  std::string strMemNick = strAccount.substr(0, nIndx);
// 			  list.push_back(MUCListItem(JID(strAccount.c_str()), RoleNone, AffiliationNone, strMemNick.c_str()));
// 		  }
// 
// 	  }
// 
...
// 	  m_parent->send( iq, this, InitGroupMembers );
  ...
  void MUCRoom::RequestRoomConfigEx(const std::string& strUniqueRoomName)
  {
	  //if( !m_parent || !m_joined )
		//  return;

	  std::string strTo = strUniqueRoomName;
	  strTo += std::string("@muc.test-d");
	  JID jidServer(strTo);
	  

	  IQ iq( IQ::Get, jidServer.bareJID()/*m_nick.bareJID() */);
	  iq.addExtension( new MUCOwner( MUCOwner::TypeRequestConfig ) );
  ...
  void MUCRoom::SetRoomConfigEx( DataForm* form)
  {
	  IQ iq( IQ::Set, m_nick.bareJID() );
	  iq.addExtension( new MUCOwner( MUCOwner::TypeSendConfig, form ) );
	  ...
	  case GetSpecifyRoomInfo:
		  {
			  ...
			  if (m_roomHandler)
			  {
				  m_roomHandler->HandleRoomNameReturn(this, m_flags, name, info.form());
			  } 
		  }
		  break;
      default:
  ...
  void MUCRoom::handleDiscoItems( const JID& /*from*/, const Disco::Items& items, int context )
  {
    ...
    switch( context )
    {
	  ...
	  case RequestRoomList:
		{
			m_roomHandler->HandleRoomList(this, items.items());
		}
		break;
	  case GetMembersOfRoom:
		  {
			  m_roomHandler->HandleRoomMemberList(this, items.items());
		  }
		  break;
  ...
  void MUCRoom::handleDiscoError( const JID& /*from*/, const Error* error, int context )
  {
    ...
    switch( context )
    {
      ...
      case GetRoomItems:
        ...
        break;
	  case RequestRoomList:
		  m_roomHandler->HandleRoomList(this, Disco::ItemList());
		  break;
	  case GetMembersOfRoom:
		  m_roomHandler->HandleRoomMemberList(this, Disco::ItemList());
		  break;
	  case GetSpecifyRoomInfo:
		  m_roomHandler->HandleRoomNameReturn(this, 0, EmptyString, 0);
		  break;
	  default:
        break;
    }
