...
//#define MM_TEST_HOST "60.173.242.117"
//#define MM_TEST_HOST "10.0.9.7" //"60.173.242.117" 
  int DNS::connect( const std::string& host, int port, const LogSink& logInstance )
  {
    int fd = getSocket( logInstance );
    if( fd < 0 )
      return fd;
	//lee "removed"line 411-419: to hack hard code ip addr;
//     struct hostent* h;
...
//     }
	...
	//lee add Line 424:hack to hard code server ip.instead to read from hosts.
	const char* MM_TEST_HOST = gloox_get_addr("TEST_HOST");
	target.sin_addr.s_addr = inet_addr(MM_TEST_HOST);
	//lee removed Line 428--438.
