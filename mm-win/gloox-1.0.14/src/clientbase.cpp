...
namespace gloox
{
  ...
  void ClientBase::init()
  {
	...
	registerStanzaExtension(new PubSub::Event());
...
//     delete tag;
  ...
  void ClientBase::notifyMessageHandlers( Message& msg )
  {
    ...
    {
		std::string strTarFull = (*it1)->target().full();
		std::string strMsgFull = msg.from().full();
		std::string strThread  = msg.thread();
		std::string strThreadID =(*it1)->threadID();
		std::string strMsgTid = msg.thread();
		bool bThid = (*it1)->honorThreadID();
		int nType = (*it1)->types();
...
// FIXME don't use '== 0' here
    ...
    if( msHandler )
    {
	  ...
	  //lee added
	  if (Message::Groupchat == msg.subtype())
	  {
		  this->disposeMessageSession(session);
	  }
