...
namespace gloox
{
                            ...
                            public IqHandler, private MessageHandler, private DiscoNodeHandler
  {
    public:
	   ...
	   * Add by lee
	   */
		virtual void Join(Presence::PresenceType type);
      ...
      void invite( const JID& invitee, const std::string& reason, const std::string& thread = EmptyString );
		...
		//add by lee.
	  void AddGroupMember(const std::vector<std::string>& vecMembers);

	  //by zyy.
	  void ExitFromGroup(const std::vector<std::string>& vecMembers);

		//add by lee: get room list of mine.
	  void GetRoomList();
		//add by lee: get members of this room.
	  void GetRoomMembers(std::string& strRoomID);
		//add by lee: get specify room info,eg:room name.
	  void GetRoomInfoEx(std::string& strRoomId);
      /**
      ...
      void requestRoomConfig();
	   ...
	   //add by lee.
	  void RequestRoomConfigEx(const std::string& strUniqueRoomName);
	 ...
	 //add by lee.
	  void SetRoomConfigEx( DataForm* form);
    ...
    protected://lee modified: private==>protected
#ifdef MUCROOM_TEST
    public:
#endif
      /**
