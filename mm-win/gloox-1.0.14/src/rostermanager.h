...
namespace gloox
{
                                   ...
                                   public PrivateXMLHandler
  {
    public:
	  ...
	  /**
	  * lee add:
	  * send adding friend request with sender nickname.
	  */
	  void SendAddRosterRequest(const JID& jid, const std::string& name, const std::string& msg );
	  ...
	  /**
	  * Send ack to server with myself nickname to support android show
	  */
	  void SendAckSubscriptionRequest(const JID& to, bool ack, const std::string& strMyNick);
