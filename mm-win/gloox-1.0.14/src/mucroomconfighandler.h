...
namespace gloox
{
  ...
  class MUCListItem
  {
    public:
	  ...
	  MUCListItem( const JID& jid, MUCRoomAffiliation affiliation, const std::string& nick )
		  : m_jid( jid ), m_nick( nick ), m_affiliation( affiliation ), m_role( RoleInvalid)
	  {}
  ...
  enum MUCOperation
  {
	...
	InitGroupMembers,				/**< Add by lee:Initialize group members>**/
    RequestRoomList,				/**< Add by lee:get room list>**/
	GetMembersOfRoom,				/**< Add by lee:get members of room>**/
	GetSpecifyRoomInfo,				/**< Add by lee:get room name by room id>**/
	ExitRoom,				/**< Add by zyy:exit the room>**/
  ...
  class GLOOX_API MUCRoomConfigHandler
  {
    public:
	  ...
	  /**
	  * Added by lee.
	  * This function is called when client send requestUnique Room name result back.
	  * if success, 
	  */
	  virtual void OnRequestRoomResult(MUCRoom* room, bool bSuccess, const std::string& strRoomName) = 0;


	  /**
	  * Added by lee.
	  * This function is called when client send set room new configuration result back.
	  */
	  virtual void OnCreateRoomResult(MUCRoom* room, bool bSuccess) = 0;
  };
