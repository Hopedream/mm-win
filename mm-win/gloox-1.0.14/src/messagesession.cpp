...
namespace gloox
{
  ...
  void MessageSession::handleMessage( Message& msg )
  {
    ...
    for( ; it != m_messageFilterList.end(); ++it )
      (*it)->filter( msg );
	//lwt add  //不知为何解析pubsub后的信息会在这里handlemessage丢失

    if( m_messageHandler )
	{
		std::string receipient = this->target().bare();
		if (receipient == "pubsub.test-d")
		{
			m_messageHandler->handleOrgReq(msg.m_strFromJid, msg.m_strFromNick
				, msg.m_strOrgId, msg.m_strOrgName, msg.m_strOrgType, msg.m_strReqId, msg.m_strDestJid, msg.m_strDestNick);
