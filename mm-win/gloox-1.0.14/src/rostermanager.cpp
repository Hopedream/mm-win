...
namespace gloox
{
  ...
  void RosterManager::SendAddRosterRequest(const JID& jid, const std::string& name, const std::string& msg )
  {
	  if( !jid )
		  return;
  ...
  void RosterManager::SendAckSubscriptionRequest(const JID& to, bool ack, const std::string& strMyNick)
  {
	  if( !to )
		  return;

	  gloox::StringList groupEmpty;
	  add( to, strMyNick, groupEmpty );

	  Subscription p( ack ? Subscription::Subscribed : Subscription::Unsubscribed, to.bareJID() );
	  p.SetSenderNick(strMyNick);
	  m_parent->send( p );
  ...
  void RosterManager::handleSubscription( const Subscription& s10n )
  {
    ...
    switch( s10n.subtype() )
    {
      ...
      {
        bool answer = m_rosterListener->handleSubscriptionRequest( s10n.from(), s10n.status(), s10n.GetNickName());
      ...
      {
        m_rosterListener->handleItemSubscribed( s10n.from(), s10n.GetNickName());
        break;
      }
