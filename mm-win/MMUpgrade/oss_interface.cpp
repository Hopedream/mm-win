...
{
	if (aos_http_io_initialize("MMUpgrade", 0) != AOSE_OK) {
		exit(1);
	}
}
...
std::string ossInterface::getEndpoint()
{
	return m_strEndpoint;
}

std::string ossInterface::getBucketName()
{
...
void ossInterface::setAcessID( std::string& strAcessID)
{
	m_strAcessID = strAcessID;
}

void ossInterface::setAcessIDSecret( std::string& strAcessIDSecret)
{
	m_strAcessIDSecret = strAcessIDSecret;
}

void ossInterface::setAcessEndpoint( std::string& strEndpoint)
{
	m_strEndpoint = strEndpoint;
}

void ossInterface::setBucketName( std::string& strBucketName)
{
	m_strBucketName = strBucketName;
}

std::string ossInterface::getAcessID()
{
	return m_strAcessID;
}

std::string ossInterface::getAcessIDSecret()
{
	return m_strAcessIDSecret;
}

int ossInterface::createBucket(std::string& strbucketName, std::string& strEndPoint,oss_acl_e acl)
{
	aos_status_t* s;
	aos_table_t* resp_headers;
	aos_string_t bucket;

	aos_str_set(&bucket,strbucketName.data());
	s= oss_create_bucket(m_pOptions,&bucket,acl,&resp_headers);
	return s->code;
}

int ossInterface::uploadObjectFromFile( std::string& strObjectName, std::string& strFilePath )
{
	aos_status_t *s;
	aos_table_t *headers;
	aos_table_t *resp_headers;
...
int ossInterface::downloadObjectByUrl(std::string & strUrl, std::string& strFilePath, int64_t filesize, std::string &fileFullName, std::string& strKey, std::string& strOpposite)
{
	double iProcess = 0;
	aos_status_t *s;
	aos_table_t *headers;
	...
	do 
	{
			...
			it != m_listHandlers.end(); ++it)
		{
			...
			if (NULL != pHandler)
			{
				if (iProcess >= 100)
					iProcess = 98;
				pHandler->OnProcess(fileFullName, iProcess, strOpposite);
			}
		}
	...
	if (s->code == OSS_SUCCESS)
	{
			...
			it != m_listHandlers.end(); ++it)
		{
			...
			if (NULL != pHandler)
			{
				pHandler->OnProcess(fileFullName, 99, strOpposite);
			}
		}
	}
	return s->code;
...
std::string ossInterface::getFileUrl( std::string &strObjectName )
{
...
int ossInterface::uploadPartFromFile( std::string& strObject, std::string& strFilePath, int64_t filesize, std::string& strKey, std::string& strOpposite)
{
	aos_status_t *s;
	aos_table_t *headers;
	aos_table_t *resp_headers;
...
int ossInterface::abord_upload( std::string &strObject, std::string &strUpload_id )
{
	aos_status_t *s;
	aos_table_t *resp_headers;
	aos_string_t bucket;
...
int ossInterface::uploadPartFromFileEx(std::string& strObject, std::string& strFilePath, int64_t filesize, std::string& strKey, std::string& strOpposite)
{
	aos_status_t *s;
	aos_table_t *headers;
	aos_table_t *resp_headers;
