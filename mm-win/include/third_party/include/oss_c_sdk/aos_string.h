...
static void aos_string_tolower(aos_string_t *str)
{
    int i = 0;
    while (i < str->len) {
...
static char *aos_strlchr(char *p, char *last, char c)
{
    while (p < last) {
        if (*p == c) {
            return p;
...
static int aos_is_quote(char c)
{
    return c == '\"';
}

static int aos_is_space(char c)
{
    return ((c == ' ') || (c == '\t'));
}

static int aos_is_space_or_cntrl(char c)
{
    return c <= ' ';
}
